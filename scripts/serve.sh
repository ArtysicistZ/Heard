#!/usr/bin/env bash
#
# serve.sh — Unified Heard service launcher
#
# Usage:
...
# .env holds shared API keys; .env.development / .env.production hold
# DATABASE_URL and AUTH_SECRET for the matching Supabase project.

if $DEV_MODE; then
    ENV_OVERRIDE="$REPO_ROOT/.env.development"
else
    ENV_OVERRIDE="$REPO_ROOT/.env.production"
fi
if [ -f "$ENV_OVERRIDE" ]; then
    set -a
    source "$ENV_OVERRIDE"
    set +a
    echo "✓ Loaded $(basename "$ENV_OVERRIDE")"
fi
...
if ! { \
    ...
    echo "✗ No Heard config file found."
...
# ── Sync DATABASE_URL + BETTER_AUTH_SECRET to frontend/.env.local ───────────
# These may come from .env.development or .env.production and must be
# forwarded so Next.js (better-auth) connects to the correct Supabase project.

for _key in DATABASE_URL BETTER_AUTH_SECRET; do
    _val="${!_key}"
    if [ -n "$_val" ]; then
        if [ -f "$FRONTEND_ENV_LOCAL" ] && grep -q "^${_key}=" "$FRONTEND_ENV_LOCAL"; then
            sed -i.bak "s|^${_key}=.*|${_key}=${_val}|" "$FRONTEND_ENV_LOCAL" && rm -f "${FRONTEND_ENV_LOCAL}.bak"
        else
            echo "${_key}=${_val}" >> "$FRONTEND_ENV_LOCAL"
        fi
    fi
done
...
# 2. Gateway API
run_service "Gateway" \
    "cd backend && PYTHONPATH=.:packages/harness uv run uvicorn app.gateway.app:app --host 0.0.0.0 --port 8001 $GATEWAY_EXTRA_FLAGS > ../logs/gateway.log 2>&1" \
    8001 30
...
echo ""
echo "=========================================="
echo "  ✓ Heard is running!  [$MODE_LABEL]"
echo "=========================================="
echo ""
