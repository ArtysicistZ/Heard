...
import { FileIcon, Loader2Icon, Volume2Icon } from "lucide-react";
...
export function MessageListItem({
  ...
  const contentText =
    ...
    "";
  const showTTS = !isHuman && contentText.length > 100;
  const [ttsVisible, setTtsVisible] = useState(false);
  const { isLoading: ttsLoading, audioUrl, error: ttsError, speak, stop: stopTTS } = useTTS();
  ...
  return (
    ...
    >
      ...
      {ttsVisible && (
        ttsError ? (
          <div className="mt-1 rounded-lg bg-red-500/10 px-3 py-2 text-sm text-red-400">
            {ttsError}
          </div>
        ) : (
          <AudioPlayer
              ...
              <Button
                ...
                onClick={() => {
                  if (ttsVisible) {
                    ...
                    setTtsVisible(true);
                    void speak(contentText.slice(0, 5000));
                  }
                }}
              >
                <Volume2Icon size={12} />
