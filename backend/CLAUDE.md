...
This is the **Heard** backend — the Python/LangGraph service powering the Heard civic empowerment platform (Claude × Penn AI Hackathon 2026, First Place, Democratic Governance track).

The backend is a full-stack "super agent" system with sandbox execution, persistent memory, subagent delegation, and extensible tool integration — all operating in per-thread isolated environments. The agent harness under `packages/harness/deerflow/` is derived from [DeerFlow](https://github.com/bytedance/deer-flow) (ByteDance's open-source super-agent framework); the civic-specific Gateway routers (`app/gateway/routers/civic.py`, `grievances.py`, `dashboard.py`, etc.) are Heard's own.
